...
list_users() {
    # Проверка, что скрипт запущен от суперпользователя
    if [[ $EUID -ne 0 ]]; then
...
if [[ -n "$error_file" ]]; then
    exec 2> "$error_file"  # Перенаправляем стандартный поток ошибок в указанный файл
fi
trap "echo 'ой-ой ошибка'>&2" DEBUG  # Искуственная ошибка

# Если не переданы аргументы, выводим справку скрипта
